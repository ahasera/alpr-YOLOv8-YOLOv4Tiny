...
# Parse arguments
parser = argparse.ArgumentParser(description='Benchmark Vehicle and License Plate Detection with Preprocessing')
...
def process_image(image_path, args, ocr_method='easyocr'):
    # Always preprocess the image
    image = preprocess_image(image_path)
    
    if image is None:
        logging.error(f"Failed to load or preprocess image: {image_path}")
        return [{
            ...
            'plate_time': 0,
            'ocr_time': 0,
            'ocr_result': 'Failed to load or preprocess image',
            'confidence': 0,
...
def benchmark_folder(input_folder, args):
    ...
    df.to_csv('benchmark_results_yolov8_preprocess.csv', index=False)
    logging.info(f"Benchmark results saved to benchmark_results_yolov8_preprocess.csv. Total rows: {len(df)}")
    
    expected_rows = len(images) * 2
    if len(df) != expected_rows:
        logging.warning(f"Expected {expected_rows} rows, but got {len(df)} rows. Some images might have multiple detections.")
